@HCAInsertPrevious
Feature: HCA Note Writer Insert Previous Jira Scenarios
#  ALM Path: Note Writer (PK)>> 009 - Insert Previous >> JIRA Scenarios
  ...
  Scenario: Insert Previous should get enabled from HP to PN[DEV-63266]
    ...
    And I click the "OK" button in the "Information" pane if it exists
    And I verify if "Exam Target" insert previous link is enabled in the "Note Writer" pane
    ...
    And I click the "Insert Selection QTV2" button in the "Data To Insert From Previous Notes" pane
    And the text "Chest pain radiating to arm" should appear in the "Note Writer" pane
 ...
 #      |Kung Fu Panda an animated movie              |
    ...
    And I click the "Bold" key in the "PlanQTV2" rich text field in the "Note Writer" pane
    ...
    And the following text should appear in the "Note Writer" pane
      ...
      | No masses, no thyromegaly, no abnormal cervical nodes, trachea midline.          |
 #      |Kung Fu Panda an animated movie              |
    And the text "Type="hidden" onblur="return HE(event,this)" FormData="true" onchange="return HE(event,this)" />" should not appear in the "Note Writer" pane
    And I select the note "A/P" section
    And I click the "OK" button in the "Information" pane if it exists
    And I click the "Problem List" insert previous link in the "Note Writer" pane
    And I click the "Insert Selection QTV2" button in the "Data To Insert From Previous Notes" pane
  ...
  # Test Case: DEV-64561_AP text shouldnt be bold in Insert Previous
    ...
    And I click the "Problem List" insert previous link in the "Note Writer" pane
    And I click the "Insert Selection QTV2" button in the "Data To Insert From Previous Notes" pane
    Then I verify the text "Burn" is plaintext in other mode in the "Note Writer" pane
    And I click the "Note Writer Cancel Note" button in the "Clinical Note" pane
    Then I click the "Yes" button in the "Question" pane
...
#     Test case: DEV-69249_Co-Sig- Multi lines text displayed in the single line; When co-signer used insert previous.
    ...
    And I wait "2" seconds
    And I click the "SearchImage" element
...
#    Test case: DEV-69249_Co-Sig- Multi lines text displayed in the single line; When co-signer used insert previous.
    ...
    And I click the "Insert Selection QTV2" button
...
#  Test case: DEV-68224_Insert Previous Popup cut off when  set Clinical Data Percent Width in Pop-Out Mode 70 percent
    ...
    And I load the "HCA History and Physical" template note for the selected patient
    And I select the note "A/P" section
    And I enter "B99.9" in the "Diagnosis Search" field in the "NoteWriter AP DXS earch" pane
    And I enter "L98.8" in the "Diagnosis Search" field in the "NoteWriter AP DXS earch" pane
    ...
    And I load the "HCA History and Physical" template note for the selected patient
    And I select the note "A/P" section
    And I enter "R07.89" in the "Diagnosis Search" field in the "NoteWriter AP DXS earch" pane
    And I enter "R51" in the "Diagnosis Search" field in the "NoteWriter AP DXS earch" pane
  ...
  Scenario Outline: Verify Previous Popup cut off when set Clinical Data Percent Width in Pop-Out Mode 70,60 and 50 percent[DEV-68224]
    ...
    Then the following text should appear in the "PopOut InsertPrevious" pane
      | Other chest pain |
      | Headache         |
    And I click the "Page Previous Image" element in the "PopOutInsertPrevious" pane
    And I wait "2" seconds
    Then the following text should appear in the "PopOutInsertPrevious" pane
      | Unspecified infectious disease                                |
      | Other specified disorders of the skin and subcutaneous tissue |
    And I click the "Page Next Image" element in the "PopOutInsertPrevious" pane
    Then the following text should appear in the "PopOutInsertPrevious" pane
      | Other chest pain |
      | Headache         |
    And I click the "Insert Selection QTV2" button in the "PopOutInsertPrevious" pane
    Then the following text should appear in the "Popout Note Wizard" pane
      | Other chest pain |
      | Headache         |
    And I click the "Save as Draft" button in the "PopoutWizard" pane
...
#   Test case: DEV-69093_Free text problems are duplicating in Discharge Summary when using Insert Previous
    ...
    And I select the note "A/P" section
    And I click the "Add Problem" button in the "Note Writer" pane if it exists
    And I enter "aaaaa" in the "Diagnosis Search" field in the "Note Writer History DX Search" pane
    And I click the "Add as Free Text" link in the "Note Writer History DX Search" pane
    And I enter "bbbbb" in the "Diagnosis Search" field in the "Note Writer History DX Search" pane
    And I click the "Add as Free Text" link in the "Note Writer History DX Search" pane
    ...
    And I wait "5" seconds
    And I select the note "Problems/Procedures" section in the "Popout Wizard" pane
  ...
  #  Test case:  DEV-69191_Addendum insert previous link  multi line added addendum visible in single line in on Progress Note Template (only template where insert previous appears in addendum flow)
    ...
    And I click the "Addendum Comments ABCQTV2" element in the "Note Writer" pane
    ...
    And I click the "SubmitOk_Addendum" button
    When I sort the "Clinical Notes" table chronologically by the "Date/Time" column in "Descending" order
    And I select "Progress Note - with Addendum" from the "Note Type (\d of \d)" column in the "Clinical Notes" table
    And the text "Final" should appear in the "HCA Progress Note" pane
    And I click the "Add Addendum" button
    And I wait "5" seconds
    And I click on the element "ClinicalDataAddendumInsertPrevious" in the "Note Writer Main" pane
    Then the following text should appear in the "Data To Insert From Previous Notes" pane
      | THIS IS FIRST LINE  |
      | THIS IS SECOND LINE |
      | 1                   |
      | 2                   |
    And I click the "AddendumInsertSelected" button
    Then the following text should appear in the "Note Writer" pane
      | THIS IS FIRST LINE  |
      | THIS IS SECOND LINE |
      | 1                   |
  ...
  Scenario: Verify horizontal scroll bar should not appear when user does insert Previous in ROS, Exam fields that contain non-breakable space long text
   #Test Case: DEV-70525_Field width change with horizontal scroll bar when user insert Previous of ROSExam fields that contain non-breakable space long text
    ...
    And I select the note "Objective" section
    And I wait "2" seconds
    And I click the "Exam Eyes Abnormal" element
    ...
    And I click the "ROS" insert previous link in the "Note Writer" pane
    And I click the "Insert Selection QTV2" button in the "Data To Insert From Previous Notes" pane
    Then the horizontal scrollbar should not present for the "ROS Abnormal QTV2" field of type "text_field" in the "Note Writer" pane
    And I select the note "Objective" section
    And I wait "5" seconds
    And I click the "Exam Target" insert previous link in the "Note Writer" pane
    And I click the "Insert Selection QTV2" button in the "Data To Insert From Previous Notes" pane
    Then the horizontal scrollbar should not present for the "Eyes Abnormal QTV2" field of type "text_field" in the "Note Writer" pane
    And I click the "Note Writer Cancel Note" button in the "Clinical Note" pane
    Then I click the "Yes" button in the "Question" pane
  ...
  #  Test Case: DEV-70245_QTv2-Carriage returns not respected for Smart tags; when data inserted from draft note using Insert Previous
    ...
    And I select the note "Subjective" section
    And I click the "ROS Resp Abnormal" button
    And I click the "ROS Narrative ABC" button
