...
#		And the "Clinical Data" table should have at least "2" rows
		And the following links should display in the "Note Writer Main" pane
			|Allergies      |
			|Clinical Notes |
			|Medication     |
			|Lab Results    |
			...
			|Test Results   |
			|Vitals         |
        And the "AnnotationIcon" image should be shown for following text in note Data section
			|Displayed  |			    |
			|true       |Bee sting		|
			|true       |Penicillin	    |
			|true       |Sulfonamides	|
		And the "TrashIcon" image should be shown for following text in note Data section
			|Displayed  |			    |
			|true       |Bee sting		|
			|true       |Penicillin	    |
			|true       |Sulfonamides	|
#		And the "Clinical Data" table should have at least "1" rows containing the text "Allergies"
        Then the "AllergiesView" pane should load
		...
		And the text "Allergies" should appear in the "Data" section in the "Progress Note" pane
...
#		And the "Clinical Data" table should have at least "1" rows containing the text "Clinical Notes"
      Then the "ClinicalNotes View" pane should load
		And I save the template as Draft
		...
		And I wait "2" seconds
		And the text "Draft" should appear in the "Progress Note" pane
		And the text "Clinical Notes" should appear in the "Data" section in the "Progress Note" pane
		And I delete the draft note in the "Progress Note" pane
...
#		And the "Clinical Data" table should have at least "2" rows
		...
		And the "TrashIcon" image should be shown for following text in note Data section
			...
			|true       |PT/INR			|
			|true       |MIPanel		|
#		And the "Clinical Data" table should have at least "1" rows containing the text "Lab Results"
      Then the "LabsView" pane should load
		...
		And I wait "2" seconds
		And the text "Draft" should appear in the "Progress Note" pane
		And the text "Lab Results" should appear in the "Data" section in the "Progress Note" pane
		And I delete the draft note in the "Progress Note" pane
...
#		And the "Clinical Data" table should have at least "2" rows
		...
		And the "TrashIcon" image should be shown for following text in note Data section
			...
			|true       |Carvedilol		|
			|true       |acetaminophen	|
#		And the "Clinical Data" table should have at least "1" rows containing the text "Medication"
      Then the "MedicationsView" pane should load
		...
		And I wait "1" second
		And the text "Draft" should appear in the "Progress Note" pane
		And the text "Medication" should appear in the "Data" section in the "Progress Note" pane
		And I delete the draft note in the "Progress Note" pane
...
#		And the "Clinical Data" table should have at least "1" rows containing the text "Test Results"
      Then the "TestResults View" pane should load
		#removed few steps as per the updated note writer step definition and added below step
		And I save the template as Draft
		And I select "Clinical Notes" from clinical navigation
		And I wait "2" seconds
		...
		Then the "Progress Note" pane should load
		And the text "Draft" should appear in the "Progress Note" pane
		And the text "Test Results" should appear in the "Data" section in the "Progress Note" pane
		And I delete the draft note in the "Progress Note" pane
...
#		And I select "Progress Note" from the "Note Type" column in the "Clinical Notes" table
		...
		And I click the "SearchText" image
		Then I doubleClick on the searchtext in the "SearchDetail" pane
		And I click the "Add Selected Text" element in the "Progress Note" pane
		Then the following fields should display in the "TestSelectionNw" pane
			|Name                 |Type    |
			|Remove Selected Text |element |		
		And I load the "Progress Note" template note for the selected patient
...
#		And I click the "Yes" button in the "Question" pane
		...
		And the "Progress Note" pane should not load
...
#		And the "Clinical Data" table should have at least "1" rows containing the text "Orders"
      Then the "OrdersView" pane should load
		...
		Then the "Progress Note" pane should load
		And the text "Draft" should appear in the "Progress Note" pane
		And the text "Orders" should appear in the "Data" section in the "Progress Note" pane
		And I delete the draft note in the "Progress Note" pane
...
#		And the "Clinical Data" table should have at least "1" rows containing the text "Clinical Notes"
#		And the "Clinical Data" table should have at least "1" rows containing the text "Lab Results"
#		And the "Clinical Data" table should have at least "1" rows containing the text "Medications"
#		And the "Clinical Data" table should have at least "1" rows containing the text "Test Results"
#		And the "Clinical Data" table should have at least "1" rows containing the text "Orders"
        Then the "Allergies View" pane should load
        Then the "ClinicalNotes View" pane should load
        Then the "Labs View" pane should load
        Then the "Medications View" pane should load
        Then the "TestResults View" pane should load
        Then the "Orders View" pane should load
		And I save the template as Draft
		When I select "Clinical Notes" from clinical navigation
		Then rows starting with the following should appear in the "Clinical Notes" table
			|Date/Time             |Note Type            |Author    |
